...
export class EnhancedPaymentService {
  ...
  private async withRetry<T>(
    ...
    const { maxAttempts, backoffMs, backoffMultiplier } = {
        ...
        if (XENDIT_CONFIG.isProduction) {
          ...
          invoice = {
            ...
            expiry_date: new Date(
              Date.now() + XENDIT_CONFIG.defaultInvoiceDuration * 1000
            ).toISOString(),
          };
    ...
    try {
      const settlementData: SettlementData = {
        ...
        settlementDate: payload.settlement_date
          ? new Date(payload.settlement_date)
          : new Date(),
    ...
    return this.withRetry(
      async () => {
        ...
        if (XENDIT_CONFIG.isProduction && payment.xenditId) {
          try {
            ...
            switch (payment.method) {
              ...
              case PaymentMethod.EWALLET:
                ...
                break;
              default:
                logger.warn('Unknown payment method for status check', {
                  paymentId,
          ...
          logger.info('Using mock status check', {
            paymentId,
            method: payment.method,
          });
        }
  ...
  async bulkStatusCheck(
    ...
    logger.info('Bulk status check completed', {
