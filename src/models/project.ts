  ...
  Project.init({
    ...
    hooks: {
      beforeUpdate: (project: Project) => {
        if ((project.currentAmount as any) >= (project.targetAmount as any) && project.status === ProjectStatus.ACTIVE) {
          project.status = ProjectStatus.CLOSED;
        }
        if (new Date() > project.endDate && project.status === ProjectStatus.ACTIVE) {
          project.status = ProjectStatus.CLOSED;
        }
      }
    }
